#!/usr/bin/env bash
# vendor/recipes/openbao/build.sh
# Build OpenBao using ko — produces a distroless OCI image.
# OpenBao is an Apache 2.0 fork of HashiCorp Vault (BSL-blocked).
...
# OpenBao build flags — these match the upstream Makefile for reproducible output
export CGO_ENABLED=0

echo "[openbao] Building OpenBao with ko..."

# OpenBao (vault fork) main package is at the module root.
# The binary is named 'bao' in the openbao project.
ko build \
...
echo "[openbao] Build complete: ${KO_DOCKER_REPO}:${IMAGE_TAG}"
