#!/usr/bin/env bash
# vendor/recipes/sealed-secrets/build.sh
# Build sealed-secrets using ko — produces a distroless OCI image.
...
echo "[sealed-secrets] Building controller with ko..."

# sealed-secrets has two binaries: the in-cluster controller and the kubeseal CLI.
# Only the controller is an OCI image; kubeseal is a local CLI tool.
ko build \
...
echo "[sealed-secrets] Build complete: ${KO_DOCKER_REPO}:${IMAGE_TAG}"
