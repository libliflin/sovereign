...
| Keycloak | 2 | Apache 2.0 | Apache 2.0 | Tier 2 exception | Red Hat steers the project; documented exit: Dex (CNCF) or Authentik if Red Hat changes terms |
| GitLab | 2 | MIT (CE) | MIT | Acceptable | GitLab Inc. controls roadmap; exit path: Gitea (MIT) or Forgejo (AGPL — review needed); CE edition only |
